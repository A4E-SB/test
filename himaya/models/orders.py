...
def create(db: Database, customer_id: int, product: str, price: float,
           ...
           shipping_cost: float = 0.0, notes: str = "", order_date: str = "") -> int:
    cur = db.execute(
        """INSERT INTO orders(customer_id, product, price, status, delivery_method,
                              wilaya, date, shipping_cost, notes)
           VALUES(?,?,?,?,?,?,?,?,?)""",
        (customer_id, product.strip(), float(price or 0), status, delivery_method,
         wilaya, order_date or date.today().isoformat(), float(shipping_cost or 0), notes),
    )
